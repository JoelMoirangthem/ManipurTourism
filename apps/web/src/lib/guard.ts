...
import type { EvidenceBundle, PlaceClaim, PlaceRecord } from "./ports";
import { fieldClass, projectFreshness } from "./domain";

export interface GuardResult {
  ok: boolean;
  /** Empty when ok. Named so the caller can log/observe rather than swallow. */
  reasons: string[];
}
...
export function citedSentences(answer: string): { sentence: string; ids: string[] }[] {
...
const PROHIBITED: { rx: RegExp; label: string }[] = [
...
export function prohibitedClaims(answer: string): string[] {
  const hits: string[] = [];
  for (const { sentence } of citedSentences(answer)) {
    if (NEGATION.test(sentence)) continue;
    for (const p of PROHIBITED) if (p.rx.test(sentence)) hits.push(p.label);
  }
  return [...new Set(hits)];
}
 ...
 */
export function staleOperationalClaims(answer: string, bundle: EvidenceBundle, now = new Date()): string[] {
  const hits: string[] = [];
  const byId = new Map<string, { claim: PlaceClaim; place: PlaceRecord }>();
  for (const place of bundle.places) {
    for (const claim of place.claims) byId.set(claim.id, { claim, place });
  }
  ...
  for (const { sentence, ids } of citedSentences(answer)) {
    if (!OPERATIONAL.test(sentence)) continue;
    ...
    for (const id of ids) {
      ...
      if (fieldClass(entry.claim.fieldKey) === "structural") continue;
      ...
      const key = `${id}:${fresh}`;
      if (seen.has(key)) continue; // report each claim once, not once per sentence
      seen.add(key);
      hits.push(key);
    }
  }
 ...
 */
export function unsupportedCitations(answer: string, bundle: EvidenceBundle): string[] {
  const byId = new Map<string, { claim: PlaceClaim; place: PlaceRecord }>();
  for (const place of bundle.places) {
    for (const claim of place.claims) byId.set(claim.id, { claim, place });
  }
  ...
  const QUALIFYING =
    /\b(record|records|unknown|not (give|state|listed|recorded|confirm)|no (data|information|record)|freshness|do(es)? not|cannot confirm|can'?t confirm|unconfirm|verify|confirm|check|ask|vary|varies|unverified|out of date|expired)\b/i;
  ...
  for (const { sentence, ids } of citedSentences(answer)) {
    ...
    if (ATTRIBUTION.test(sentence)) continue; // (c)
    ...
    if (QUALIFYING.test(sentence)) continue;
    ...
    for (const id of ids) {
      ...
      if (!namesPlace && !enough) bad.push(`${id}:weak-support`);
    }
  }
  return [...new Set(bad)];
}
...
export function validateAnswer(answer: string, bundle: EvidenceBundle, now = new Date()): GuardResult {
  const reasons: string[] = [];
  const missing = unsupportedCitations(answer, bundle);
  if (missing.length) reasons.push(...missing.map((m) => `unsupported-citation:${m}`));
  const prohibited = prohibitedClaims(answer);
  if (prohibited.length) reasons.push(...prohibited.map((p) => `prohibited:${p}`));
  const stale = staleOperationalClaims(answer, bundle, now);
  if (stale.length) reasons.push(...stale.map((s) => `stale-as-current:${s}`));
